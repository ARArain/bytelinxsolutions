...
if (!function_exists('get_browser_name')) {
    ...
    function get_browser_name($user_agent){
        ...
        elseif (strpos($t, 'firefox'   ) || strpos($t, 'Mozilla')  ) return 'Firefox'          ;   
...
if (!function_exists('general_asset')) {
     ...
     * @return string
     */
    function general_asset($path)
    {
        if(get_browser_name($_SERVER['HTTP_USER_AGENT']) == 'Safari'){
            $pathArray = explode('/',$path);
            $preFile = $pathArray[count($pathArray)-1];
            $newFile = explode('.',$preFile);
            $fileName = $newFile[0];
            $fileExtention = $newFile[1];
            if($fileExtention == 'webp'){
                $newFileName = $fileName . '.png';
                $pathArray[count($pathArray)-1] = $newFileName;
